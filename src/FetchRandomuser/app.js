...
class FetchRandomuser extends React.Component {
    ...
    render() {
        ...
        return (
            <div style={styleuser} className = "fetchuser">{this.state.loading || !this.state.randomuser ?
                (<div>loading..</div>) :
                (<div >
                    {this.state.randomuser.map(data => {
                        return (<section key={data.name.first.toString()}>
                            <img alt="profile-pic" src={data.picture.medium} />
                            <div className="border" />
                            <h4>Country : {data.location.country}</h4>
                            <span>Full Name : {data.name.first}{data.name.last}</span>
